...
class WorldMap extends Component {
    ...
    componentDidMount(){
        ...
        L.tileLayer('htt://api.mapbox.com/styles/v1/osmallfrogo/cjumipd087qvo1ftqrkq7hcxb/tiles/256/{z}/{x}/{y}@2x?access_token=pk.eyJ1Ijoib3NtYWxsZnJvZ28iLCJhIjoiY2p0em5pNnZ3MzZjMTRlbXVyOTNyYjJ5aiJ9.rUrqX8nYoZXe0mxMowBLyQ', {
            ...
            function drawGrid(){
                ...
                Promise.all([
                    ...
                    for (let ii = 0; ii < type.length; ii++) {
                        for (let jj = 0; jj <  1/*type[ii].length*/; jj++) {
                            let grid = type[ii][jj];
                            ...
                            L.circle([cx,cy],{radius:ir,color:incolor,opacity:0.78,fill:true,fillOpacity:0.78/*flowSum()/90000+0.1*/,stroke:false}).addTo(global.map);
                            ...
                            if(mapLevel>=13){
                                ...
                                for(let myI = 0;myI<24;myI++){
                                    ...
                                    let a1 = a-0.00005*myI*Math.random();
                                    let b1 = b+0.00001*myI*Math.random();
                                    // if(asterData["data"]["data"][numGrid][myI][1]>asterData["data"]["data"][numGrid][myI][2]){
                                    if(a1>b1){
                                        inCurve = L.curve(describeArc(cy, cx, 3.5, 7.9, angs+myI*anStep, ange+myI*anStep, a1/*asterData["data"]["data"][numGrid][myI][1]*/),
                                            {color: ocolor[5], fill: true,stroke:true,fillColor:'#5732a8',fillOpacity:1,weight:1});
                                        outCurve = L.curve(describeArc(cy, cx, 3.5, 7.9, angs+myI*anStep, ange+myI*anStep, b1/*asterData["data"]["data"][numGrid][myI][2]*/),
